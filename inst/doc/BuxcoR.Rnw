...
@
Note that at this point the data in the database consists of values directly parsed from the raw file. Adding additional labels or computations can be done through the \texttt{addAnnotation} method by specifying the \texttt{BuxcoDB} object and a function returning the SQL query.  There are currently two functions that are defined, though users can define their own as well with some familiarity of the database structure.  The first, \texttt{day.infer.query} computes the number of days past the first measurement for a given animal.  The second \texttt{break.type.query} labels the measurements as 'ACC' for acclimation, 'EXP' for experimental, 'UNK' for unknown (where there is only one set of measurements for a given animal and given time point) or 'ERR' which likely represents an error in the parsing or query. Examples are shown below.
...
@
The index argument specifies whether an index should be added to the columns.  There should not really be any harm in adding them at each stage though as the method tries to create a set of covering indices to maximize retrieval speed,  I would recommend waiting until the last call to \texttt{addAnnotation}.  Again we can extract the data as before using \texttt{retrieveData} though this time in order to specify constraints on any column defined through \texttt{addAnnotation} we need to type in the column name and the constraint vector at the end.\\\\For this it is useful to figure out the names and different values of the annotation columns.
